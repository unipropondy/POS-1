...
const getReportDateWhereSql = (
  filter = "daily",
  saleDateColumn = "sh.LastSettlementDate",
) => {
...
const normalizePayMode = (paymentMethod = "CASH") => {
  const raw = String(paymentMethod || "CASH")
    .toUpperCase()
    .trim();
  if (["CAS", "CASH"].includes(raw)) return "CASH";
  if (["CARD", "VISA", "MASTER", "MASTERCARD", "AMEX", "DINERS"].includes(raw))
    return "CARD";
...
const validateSalePayload = ({ totalAmount, paymentMethod, items }) => {
  ...
  for (let i = 0; i < items.length; i += 1) {
    ...
    if (!dishId && !dishName)
      return `Item ${i + 1} is missing dish information`;
    if (!Number.isFinite(qty) || qty <= 0)
      return `Item ${i + 1} has invalid quantity`;
    if (!Number.isFinite(price) || price < 0)
      return `Item ${i + 1} has invalid price`;
  }
...
router.get("/transactions", async (req, res) => {
  try {
    ...
    const result = await pool
      ...
      .input("End", sql.DateTime, endDate || new Date()).query(`
...
router.get("/range", async (req, res) => {
  try {
    ...
    const result = await pool
      ...
      .input("End", sql.DateTime, endDate).query(`
...
router.get("/category", async (req, res) => {
  try {
    ...
    const result = await pool
      ...
      .input("OrderTypes", sql.VarChar(200), orderTypes.join(",")).query(`
        WITH AppItemRows AS (
          SELECT
...
router.get("/category", async (req, res) => {
  try {
    ...
    const appDateWhereSql = getReportDateWhereSql(
      filter,
    ...
    console.log(
      `[REPORT API] type=category filter=${filter} rows=${result.recordset.length}`,
    );
    res.json(result.recordset || []);
  } catch (err) {
...
router.get("/dish", async (req, res) => {
  try {
    ...
    console.log(
      `[REPORT API] type=dish filter=${filter} rows=${result.recordset.length}`,
    );
    res.json(result.recordset || []);
  } catch (err) {
...
router.get("/daily-order-count", async (req, res) => {
  try {
    ...
    const result = await pool
      ...
      .input("End", sql.DateTime, endOfDay).query(`
...
router.post("/save", async (req, res) => {
  try {
    ...
    console.log(
      `[SAVE SALE] Starting transaction. Items: ${items?.length}, Total: ${totalAmount}, Method: ${paymentMethod}`,
    );
    ...
    try {
      ...
      console.log(
        `[SAVE SALE] SettlementHeader success. OrderId: ${generatedOrderId}`,
      );
      ...
      if (items && Array.isArray(items)) {
        for (const item of items) {
          console.log(
            `[SAVE SALE] Step 4: Item [${item.dish_name || item.name}]...`,
          );
          const dishId = toGuidOrNull(item.dishId || item.id);
          const dishMeta = await transaction
            .request()
          ...
          await transaction
            ...
            .input(
              "DishId",
              sql.UniqueIdentifier,
              toGuidOrNull(meta.DishId || dishId),
            )
            .input(
              "DishGroupId",
              sql.UniqueIdentifier,
              toGuidOrNull(meta.DishGroupId),
            )
            .input(
              "SubCategoryId",
              sql.UniqueIdentifier,
            ...
            .input(
              "CategoryId",
              sql.UniqueIdentifier,
              toGuidOrNull(meta.CategoryId),
            )
            .input(
              "DishName",
              sql.NVarChar(255),
              item.dish_name || item.name || "Unknown",
            )
      ...
      await transaction
        ...
        .input("ModifiedOn", sql.DateTime, new Date()).query(`
      ...
      if (memberId && (paymentMethod || "").toUpperCase() === "CREDIT") {
        await transaction
          ...
          .query(
            `UPDATE MemberMaster SET CurrentBalance = CurrentBalance + @Amount WHERE MemberId = @MemberId`,
          );
      }
      const express = require("express");
      const router = express.Router();
...
`;
      ...
      const normalizePayMode = (paymentMethod = "CASH") => {
        ...
        if (["CAS", "CASH"].includes(raw)) return "CASH";
        if (
          ["CARD", "VISA", "MASTER", "MASTERCARD", "AMEX", "DINERS"].includes(
            raw,
          )
        )
          return "CARD";
    ...
    `);
      ...
      router.get("/detail/:id", async (req, res) => {
        try {
          ...
          const result = await pool
            ...
            .input("Id", sql.UniqueIdentifier, req.params.id)
            .query(
              "SELECT * FROM SettlementItemDetail WHERE SettlementID = @Id",
            );
          res.json(result.recordset);
        } catch (err) {
          res.status(500).json({ error: err.message });
      ...
      router.get("/category", async (req, res) => {
        try {
          ...
          const applyOrderTypeFilter =
            orderTypes.length > 0 && orderTypes.length < 2;
      ...
      router.get("/category", async (req, res) => {
        try {
          ...
          const legacyDateWhereSql = getReportDateWhereSql(
            filter,
            "InvoiceDate",
          );
          console.log(`[REPORT API] type=category filter=${filter}`);
        ...
        `);
            let businessUnitId =
              bizRow.recordset.length > 0
                ...
                : DEFAULT_GUID;
            console.log(
              `[SAVE SALE] BusinessUnitId resolved to: ${businessUnitId}`,
            );
          ...
          `);
            ...
            console.log(
              `[SAVE SALE] Step 3: Inserting SettlementTotalSales...`,
            );
            await transaction
              .request()
              .input("SettlementID", sql.UniqueIdentifier, settlementId)
      ...
      router.get("/orders/check/:orderId", async (req, res) => {
        try {
          ...
          const result = await pool
            ...
            .query(
              "SELECT SettlementID FROM SettlementHeader WHERE OrderId = @OrderId AND IsCancelled = 0",
            );
      ...
      router.post("/orders/validate-cancel", async (req, res) => {
        try {
          ...
          const result = await pool
            ...
            .query(
              "SELECT IsCancelled FROM SettlementHeader WHERE SettlementID = @Id",
            );

          if (result.recordset.length === 0)
            return res
              .status(404)
              .json({ valid: false, message: "Order not found" });
          if (result.recordset[0].IsCancelled)
            return res
              .status(400)
              .json({ valid: false, message: "Order is already cancelled" });
      ...
      router.get("/payment-detail/:payMode", async (req, res) => {
        try {
          ...
          const result = await pool
            ...
            .query(
              "SELECT * FROM [dbo].[Paymode] WHERE LTRIM(RTRIM(PayMode)) = @PayMode AND Active = 1",
            );
